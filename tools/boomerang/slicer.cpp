...
public:
  ...
  bool isZero() const { return i == nullptr; }
...
public:
  IFDSSLicer(LLVMBasedICFG *icfg, const LLVMTypeHierarchy *th,
             const ProjectIRDB *irdb, const LLVMPointsToInfo *PT,
             const std::set<const llvm::Value *> sc,
             const std::vector<Term> *terms)
      : IFDSTabulationProblem<const llvm::Instruction *, SlicerFact,
                              const llvm::Function *, const llvm::StructType *,
                              const llvm::Value *, LLVMBasedICFG>(irdb, th,
                                                                  icfg, PT),
        slicingCriteria(sc) {}
...
private:
  ...
  map<const Instruction *, set<SlicerFact>> initialSeeds() override {
    ...
    set<SlicerFact> facts;
    const Instruction *i = &ICF->getFunction("main")->front().front();
...
int main(int argc, const char **argv) {
  ...
  LLVMBasedCFG CFG;
  LLVMBasedICFG cg(DB, CallGraphAnalysisType::DTA, {}, &th);
  LLVMPointsToInfo PT(DB);
  std::ifstream i("targets/locations.json");
  json j;
  i >> j;
  ...
  IFDSSLicer slicer(&cg, &th, &DB, &PT, sc, &terms);
